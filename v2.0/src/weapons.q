...
// explode if under water
	if (self.waterlevel > 1 && match_in_progress == M_MATCH)	//team
	{
		if (deathmatch > 3)
		{
			if (random() <= 0.5)
...
//	local vector v1;
//	makevectors(self.angles);
//	spawn_tdeath(self.origin, world);
//	self.velocity = (v_forward * 1400);
//	v1 = self.origin - (v_up * 15);
//	traceline(v1, v1 + (v_forward * 160), FALSE, self);
//	v1 = trace_endpos;
//	traceline(v1, v1 + (v_forward * 30), FALSE, self);
//	if(trace_fraction < 1) v1 = trace_endpos - (v_forward * 35);
//	setorigin(self, v1 + (v_up * 15));
...
//team
	if (!W_CheckNoAmmo ())
		return;
//team
	if((!stof(infokey(world, "k_prewar")) && match_in_progress != M_MATCH) || match_in_progress == M_COUNTDOWN) {
		stuffcmd(self, "bf\n");
		if(time > (self.k_msgcount + 2)) {
			sprint(self, PRINT_CHAT, "console: you are not allowed to fire\n");
			self.k_msgcount = time;
		}
		return;
	}
	if (self.k_666 && !(self.weapon == IT_AXE || self.weapon == IT_SHOTGUN)) {
...
// TEAM -->
        ...
        if (self.impulse >= 100 && !match_in_progress)
                SelectMap();
        else if (self.impulse == IMP_REPORT) ReportMe();
