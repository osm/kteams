...
void() health_touch =
{
	...
	if (match_in_progress != M_MATCH) return;  // TEAM
...
// TEAM -->
	if (match_in_progress != M_MATCH) return;
// <-- TEAM
...
// TEAM -->
	if (match_in_progress != M_MATCH || !stof(infokey(world, "k_pow"))) return;
//	if (match_in_progress != M_MATCH) return;
...
void() item_artifact_invulnerability =
{
	...
	if(stof(infokey(world, "k_pow"))) self.effects = self.effects | EF_RED;		//team
...
void() item_artifact_super_damage =
{
	...
	self.items = IT_QUAD;
	if(stof(infokey(world, "k_pow"))) self.effects = self.effects | EF_BLUE;		//team
	setsize (self, '-16 -16 -24', '16 16 32');
	StartItem ();
};
...
void() DropBackpack =
{
	...
	if (match_in_progress != M_MATCH || !stof(infokey(world, "dp"))) return;	//team
...
//team
	if(stof(infokey(world, "k_frp"))) {
		if((self.items & IT_NAILGUN) && (self.ammo_nails > 0)) item.items = IT_NAILGUN;
		if((self.items & IT_SUPER_SHOTGUN) && (self.ammo_shells > 0)) item.items = IT_SUPER_SHOTGUN;
		if((self.items & IT_SUPER_NAILGUN) && (self.ammo_nails > 0)) item.items = IT_SUPER_NAILGUN;
		if((self.items & IT_GRENADE_LAUNCHER) && (self.ammo_rockets > 0)) item.items = IT_GRENADE_LAUNCHER;
		if((self.items & IT_LIGHTNING) && (self.ammo_cells > 0)) item.items = IT_LIGHTNING;
		if((self.items & IT_ROCKET_LAUNCHER) && (self.ammo_rockets > 0)) item.items = IT_ROCKET_LAUNCHER;
	}
//team
