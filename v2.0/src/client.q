...
// TEAM -->
	if(match_in_progress == M_MATCH) {
		...
		parm1 = IT_AXE | IT_SHOTGUN | IT_SUPER_SHOTGUN | IT_NAILGUN | IT_SUPER_NAILGUN | IT_ROCKET_LAUNCHER | IT_GRENADE_LAUNCHER | IT_LIGHTNING;
		parm2 = 100;
		parm3 = 0;
		parm4 = 100;
		parm5 = 200;
...
{
// TEAM -->
	if((time < self.suicide_time + SUICIDE_DELAY) && stof(infokey(world, "k_kfp"))) {
		sprint(self, PRINT_HIGH, "Only one suicide in SUICIDE_DELAY seconds\n");
		return;
	}
...
// <-- TEAM
	...
	logfrag (self, self);
	if(match_in_progress == M_MATCH)	//team
...
//			if (thing.classname == "player") pcount = pcount + 1;
			if(thing.classname == "player") {
				if(!(stof(infokey(world, "k_spw")) == 2 && match_in_progress == M_MATCH && !thing.k_1spawn) &&
...
//team
		if(stof(infokey(world, "k_spw")) && match_in_progress == M_MATCH && self.k_lastspawn == spot)
		 pcount = pcount + 1;
//team
        if (pcount == 0) {
...
//team
	if(match_in_progress == M_MATCH) self.k_1spawn = 60;
	k_nspots = totalspots;
//team
	totalspots = totalspots - 1;
	if (!numspots) {
...
//team
		if(k_nspots > 2 && match_in_progress == M_MATCH) self.k_lastspawn = spot;
		if(match_in_progress == M_MATCH && stof(infokey(world, "k_spw")) > 0 && !k_checkx) {
			makevectors(spot.angles);
			thing = findradius(spot.origin, 84);
			while(thing) {
				if(thing.classname == "player" && thing.health > 0) {
					v1 = thing.origin - (v_up * 15);
					traceline(v1, v1 + (v_forward * 160), FALSE, thing);
					v1 = trace_endpos;
					traceline(v1, v1 + (v_forward * 30), FALSE, thing);
...
//team
	self.mdl = self.model;
//	if(!self.k_accepted && match_in_progress == M_MATCH) {
	if(!self.k_accepted) {
...
//team
	...
	if (deathmatch == 4 && match_in_progress == M_MATCH)	//team
	{
		self.ammo_shells = 0;
...
//		self.items = self.items | IT_GRENADE_LAUNCHER;
	...
	if (deathmatch == 5 && match_in_progress == M_MATCH)	//team
	{
		self.ammo_nails = 80;
		self.ammo_shells = 30;
...
//team   autospawn
	if((time - self.dead_time) > MAX_DEAD_TIME && match_in_progress) {
...
//team
	if(match_in_progress == M_MATCH && self.k_accepted == 2 && self.k_teamnum && k_checkx) {
		...
		if(s1 != s2)
		{
			...
			stuffcmd(self, "\ndisconnect\n");
			bprint2(PRINT_HIGH, self.netname, " gets kicked for changing team\n");
			self.k_accepted = 0;
			f1 = 1;
			f2 = 0;
...
//team
	if(!self.k_accepted && match_in_progress == M_MATCH) return;
...
//kombat teams gained control over this
	...
	if(self.k_accepted == 2) {
		...
		s1 = ftos(self.frags);
		bprint2(PRINT_HIGH, s1, " frags\n");
		sound(self, CHAN_BODY, "player/tornoff2.wav", 1, ATTN_NONE);
		set_suicide_frame();
		if(match_in_progress == M_MATCH && self.ready) {
			f1 = 1;
			f2 = 0;
			while(f1 < k_userid && !f2) {
				s1 = ftos(f1);
	...
	} else if(match_in_progress == M_MATCH && !self.k_accepted) {
		self.takedamage = DAMAGE_NO;
		self.solid = SOLID_NOT;
		self.movetype = MOVETYPE_NONE;
		self.modelindex = 0;
...
///team
				if(infokey(targ,"gender") == "f") bprint(PRINT_MEDIUM," electrocutes herself.\n");
				else bprint(PRINT_MEDIUM," electrocutes himself.\n");
///team
...
// team --> no points for telefragging teammate
			if(match_in_progress != M_MATCH) {
				bprint2(PRINT_MEDIUM,targ.netname, " was telefragged\n");
				return;
			}
			...
			{
				bprint(PRINT_MEDIUM,targ.netname);
				if(infokey(targ,"gender") == "f") bprint (PRINT_MEDIUM," was telefragged by her teammate\n");
				else bprint(PRINT_MEDIUM," was telefragged by his teammate\n");
...
///team
		...
		else
		{
			...
			{
///team
				if(infokey(targ,"gender") == "f") bprint(PRINT_MEDIUM," fell to her death\n");
				else bprint(PRINT_MEDIUM," fell to his death\n");
///team
				return;
			}
